...
export const OFFICE_AGENT_TOOL_FILES_DIR_NAME = "officeagent-tool-files";
...
export const OFFICE_AGENT_TOOL_FILES_ENV_NAME = "OFFICE_AGENT_TOOL_FILES";
...
export const OFFICE_AGENT_DISABLE_GLOBAL_SQL_TOOL_ENV_NAME = "OFFICE_AGENT_DISABLE_GLOBAL_SQL_TOOL";
...
export const OFFICE_AGENT_SQLSERVER_READONLY_CONTEXT = "# Acceso a base de datos SQL Server desde Castrosua IA\n\nPara responder consultas que requieran datos de base de datos, usa la herramienta Pi **`castrosua_sql_read_only`**. La herramienta da acceso de solo lectura únicamente a la base de datos **`CastrosuaIA`** del SQL Server corporativo.\n\n## Herramienta de acceso\n\n### `castrosua_sql_read_only`\nUsar esta herramienta para:\n- comprobar la conexión (`action: \"info\"`)\n- listar vistas (`action: \"list_tables\"`, con `includeViews: true`)\n- describir columnas de una vista (`action: \"describe\"`)\n- obtener una muestra limitada de filas (`action: \"sample\"`)\n- ejecutar consultas SQL de solo lectura (`action: \"query\"`)\n\nReglas obligatorias de uso:\n- La base de datos por defecto y única soportada es **`CastrosuaIA`**. No intentes cambiar de base de datos ni pedir/configurar un parámetro de base de datos.\n- La herramienta tiene acceso de solo lectura a la base de datos **`CastrosuaIA`**. Usa `list_tables`, `describe` y `sample` para descubrir el esquema disponible cuando sea necesario.\n- Cuando el usuario pida información de ERP Logic/Sage, artículos, proveedores, albaranes o pedidos de proveedor, debes consultar esta herramienta en vez de inventar datos.\n- Antes de construir consultas complejas, usa `list_tables`, `describe` y/o `sample` si no conoces con seguridad el esquema.\n- En consultas amplias, usa `TOP`, filtros y columnas concretas para limitar resultados.\n- `action: \"query\"` solo debe usarse con consultas de solo lectura `SELECT` o `WITH`.\n\n---\n\n---\ntags:\n  - Instrucciones\n  - IA\n---\n# Mapa de bases de datos para la IA\n\nEste documento sirve para orientar a la IA sobre **qué base de datos consultar según el sistema mencionado por el usuario**.\n\n## Supernivel: bases de datos por sistema\n\n### `LOGIC`\nUsar esta base de datos cuando el usuario pregunte por:\n- **Sage**\n- **Logic**\n- **ERP**\n- **albaranes de proveedor**\n- **pedidos de proveedor**\n- **facturas de proveedor**\n- **albaranes de cliente**\n- **pedidos de cliente**\n- **facturas de cliente**\n- **proveedores**\n- **clientes**\n- **artículos**\n- **stock**\n- procesos de compras, ventas o recepción/expedición de mercancía en el ERP\n- códigos de artículo generalmente de 6 dígitos vistos en documentación técnica, listas de materiales, instrucciones de montaje o fabricación\n\n#### Interpretación general de códigos de artículo\n- Cuando el usuario aporte un código numérico de 6 dígitos en el contexto de materiales, montaje, fabricación, listas o documentación técnica, la IA debe interpretarlo por defecto como un **código de artículo de Logic** (`CodigoArticulo`), salvo que el contexto indique claramente otra cosa.\n- Expresiones como **\"código Logic\"**, **\"código artículo\"**, **\"artículo\"** o simplemente **\"Logic\"** pueden referirse al identificador de artículo del ERP.\n- Ejemplos típicos: `012496`, `209981`, `012919`, `222885`, `014568`, `220028`, `201373`, `220740`, `207727`, `010383K`, `C000054-01`, `JL-2011/014-11846`, `LP-2014/019`\n- Para la venta y fabricación de carrocerías, los códigos de estos artículos serán:\n\n|          | Castrosua | Carsa   |\n| -------- | --------- | ------- |\n| Scania   | SK xxxx   | SC xxxx |\n| Mercedes | ME xxxx   | MC xxxx |\n| Volvo    | V  xxxx   | VO xxxx |\n| Tempus   | H  xxxx   |         |\n| Iveco    | P  xxxx   | IV xxxx |\n| BYD      | BD xxxx   | BY xxxx |\n| Man      | MN xxxx   | MA xxxx |\n| Veris    | VE xxxx   |         |\nSiendo xxxx un número de 4 dígitos correlativo a ese prefijo\n\n- Si el usuario pide buscar, identificar o ampliar información sobre uno de estos códigos, la consulta debe orientarse normalmente a la tabla `Articulos` por `CodigoArticulo`.\n\n#### Empresas\n- **1**: Castrosua\n- **2**: Carsa\n- **3**: Insucar\n- **5**: Cidsa\n\n#### Almacenes\n**Castrosua**\n**101**\tEstructura\n**102**\tChapa\n**103**\tPintura\n**104**\tREMATE\n**105**\tElectricidad\n**106**\tMecánica\n**107**\tPoliéster\n**1FP**\tFabricación\n**BESC**    BESCOROMAR\n**CBUS**\tCARROCERIAS BUS\n**CEN**\tCENTRAL CASTROSUA\n**CONS**\t\tCONSMAN BARCELONA\n**DEV**\t\tAlmacén de devoluciones\n**GARA**\tALMACEN DE TRANSITO\n**GSI**\tMáquinas PRL y Brocas GSI\n**IDI**\tALMACEN DE I+D+I\n**LEV**\tAVANZA MOVILIDAD LEVANTE\n**LIT**\tUAB \"TRANSMITTO\"\n**MANT**\t\t\tMANTENIMIENTO CASTROSUA\n**PRL**\t\t\t PRL\n**PROT**\t\tPROTOTIPOS CASTROSUA\n**REC**\t\tCARROCERA CASTROSUA, S.A.\n**REG**\t\t\tREGULARIZACIONES \n**REGI**\t\t\tREGULARIZACION INVENTARIO\n**SCCA**\t\tSCANIA EN CARABANCHEL\n**SCCH**\t\tSCANIA EN SAN CHINARRO\n**TCAM**\t\tCARROCERA CASTROSUA, S.A.\n**TITF**\t\tTITSA TENERIFE\n**TNF**\t\t    TENERIFE NORTE - SC\n**TNFS**\t\tTENERIFE SUR - SC\n**WRT**\t\t\tEXPENDEDORAS WURTH CS\n\n**Carsa**\n**1**\tAlmacén CENTRAL\n**2**   PLANTA\n**201**\tEstructura\n**202**\tChapa\n**203**\tPintura\n**204**\tRemate\n**205**\tElectricidad\n**206**\tMecánica\n**207**\tPoliéster\n**210**\tAcabados\n**2DEV**    Devoluciones\n**2FP**\tFabricación Propia\n**2GAR**\tGarantías Carsa\n**2GSI**\tMaq. PRL y Brocas GSI\n**2WRT**\tEXPENDEDORAS WURTH \n**3**\tREGULARIZACIONES \n**4**\tPRL \n**5**\tMantenimiento \n**6**\tREGULARIZACION INVENTARIOS\n**CAMP**\t\tCAMPA\n**TCR**\t\t\tTránsito (Carsa)\n\n**Cidsa**\n**CID**\t\t\tCENTRAL \n**CPRL**\t\t\tAlmacen PRL \n**CREG**\t\t\tRegularizaciones \n**REGD**\t\t\tREGULARIZACION INV \n\n**Insucar**\n**IBSC**\t\t\tAlmacén Besc \n**ICEN**\t\t\tCENTRAL \n**IDEV**\t\t \tDEVOLUCIONES INSUCAR\n**IGAR**\t\t\tAlmacén de tránsito \n**IPRL**\t\t\tAlmacen PRL \n**IREG**\t\t\tREGULARIZACIONES \n**IRGI**\t\t\tREGULARIZACION INV \n**ISAT**\t\tsAT Insucar\n**ITNF**\t\t\tAlmacén Titsa Insucar\n\n#### Series de venta\nDescripción de los valores que aparecen en los campos SeriePedido, SerieAlbara y SerieFactura en las tablas de venta LOGIC_CabeceraPedidoCliente, LOGIC_LineasPedidoCliente, LOGIC_CabeceraAlbaranCliente, LOGIC_LineasPedidoCliente, LOGIC_ResumenCliente\n\n| Empresas   | CodigoCuenta    | DescripcionCuenta                   | Serie Venta | Serie Venta Rectificativa |\n| ---------- | --------------- | ----------------------------------- | ----------- | ------------------------- |\n| 1 y 2      | 700000000000000 | Ventas de mercaderías (Carrocerías) | A           | AR                        |\n| 3 y 5      | 705000000000000 |                                     | A           | AR                        |\n| 1, 2, 3, 5 | 700000000000001 | Ventas de mercaderías (Recambios)   | B           | RR                        |\n| 1, 2, 3, 5 | 700000000000002 | Venta de mercaderías (internas)     | C           | RC                        |\n| 1, 2, 3, 5 | 759000000000000 | Ingresos por servicios diversos     | D           | RD                        |\n| 1, 2, 3, 5 | 705000000000003 | Trabajos para empresas externas     | E           | RE                        |\n| 1, 2, 3, 5 |                 |                                     | G           | RG                        |\n| 1 y 2      | 705000000000000 | Prestaciones de servicios           | F           | RG                        |\n| 1, 2, 3, 5 | 705000000000002 | Prestación de servicios postventa   | P           | RP                        |\n| 1, 2, 3, 5 |                 |                                     | H           | RH                        |\n| 1, 2, 3, 5 | 759000000000000 | Ingresos por servicios diversos     | Z           | RZ                        |\n\n#### Tablas principales descritas actualmente\n\n##### `LOGIC_CabeceraAlbaranProveedor`\nGuarda la **cabecera del albarán de proveedor**, es decir, los datos generales del documento: fecha, proveedor, importes, estados, forma de pago, observaciones y otra información administrativa.\n\n```sql\nSELECT         CodigoEmpresa,  EjercicioAlbaran, SerieAlbaran, NumeroAlbaran, FechaAlbaran, CodigoProveedor, NumeroLineas, SiglaNacion, CifDni, CifEuropeo, RazonSocial, RazonSocial2, Nombre, \n\t\t\tStatusFacturado,  CodigoProyecto, CodigoSeccion, CodigoDepartamento,  ObservacionesProveedor, ObservacionesAlbaran, ObservacionesFactura, SuAlbaranNo, FechaSuAlbaran, EjercicioPedido, SeriePedido, NumeroPedido, FechaFactura, \n                         EjercicioFactura, SerieFactura, NumeroFactura, BaseImponible,  TotalCuotaIva,  ImporteLiquido\nFROM            CabeceraAlbaranProveedor\nWHERE    codigoempresa in (1,2,3,5) \n```\n\n**Clave funcional del albarán:**\n- `CodigoEmpresa`\n- `EjercicioAlbaran`\n- `SerieAlbaran`\n- `NumeroAlbaran`\n\n**Significado de los campos de la consulta:**\n- `CodigoEmpresa`: empresa del grupo a la que pertenece el albarán.\n- `EjercicioAlbaran`: año o ejercicio contable del albarán.\n- `SerieAlbaran`: serie documental del albarán.\n- `NumeroAlbaran`: número interno del albarán.\n- `FechaAlbaran`: fecha del albarán en el ERP.\n- `CodigoProveedor`: código interno del proveedor.\n- `NumeroLineas`: número de líneas del documento.\n- `SiglaNacion`: sigla o prefijo del país.\n- `CifDni`: NIF/CIF del proveedor.\n- `CifEuropeo`: identificación fiscal europea, si aplica.\n- `RazonSocial`: razón social principal del proveedor.\n- `RazonSocial2`: segunda línea de razón social o ampliación del nombre.\n- `Nombre`: nombre comercial o nombre resumido del proveedor.\n- `StatusFacturado`: indica si el albarán ha sido facturado o su estado de facturación.\n- `CodigoProyecto`: proyecto asociado, si existe imputación.\n- `CodigoSeccion`: sección asociada al documento.\n- `CodigoDepartamento`: departamento asociado al documento.\n- `ObservacionesProveedor`: observaciones generales del proveedor arrastradas al documento.\n- `ObservacionesAlbaran`: observaciones propias del albarán.\n- `ObservacionesFactura`: observaciones relacionadas con la factura.\n- `SuAlbaranNo`: número de albarán del proveedor.\n- `FechaSuAlbaran`: fecha del albarán del proveedor.\n- `EjercicioPedido`: ejercicio del pedido relacionado, si existe.\n- `SeriePedido`: serie del pedido relacionado.\n- `NumeroPedido`: número del pedido relacionado.\n- `FechaFactura`: fecha de la factura asociada.\n- `EjercicioFactura`: ejercicio de la factura asociada.\n- `SerieFactura`: serie de la factura asociada.\n- `NumeroFactura`: número de la factura asociada.\n- `BaseImponible`: importe base antes de impuestos.\n- `TotalCuotaIva`: importe total de IVA del documento.\n- `ImporteLiquido`: importe final líquido del albarán.\n\n##### `LOGIC_LineasAlbaranProveedor`\nGuarda el **detalle de líneas del albarán de proveedor**, es decir, los artículos recibidos en cada documento: artículo, descripción, cantidades, precios, descuentos, IVA, almacén e importes por línea.\n\n```sql\nSELECT        CodigoEmpresa, EjercicioAlbaran, SerieAlbaran, NumeroAlbaran, Orden, LineasPosicion, LineaPedido,  FechaRegistro, FechaAlbaran, CodigoArticulo, CodigoAlmacen, Partida, DescripcionArticulo, \n                         Descripcion2Articulo,  CodigoFamilia, CodigoSubfamilia, CodigodelProveedor, CodigoDefinicion_, CodigoProyecto, CodigoSeccion, \n                         CodigoDepartamento, StatusStock,  EjercicioPedido, SeriePedido, NumeroPedido, EjercicioFactura, SerieFactura, NumeroFactura, Unidades,  Precio, PrecioRebaje,  BaseImponible, \n\t\t\t\t\t\t  CuotaIva, \n                         TotalIva,  ImporteLiquido,  CAS_PartidaVentas\nFROM            LineasAlbaranProveedor\nWHERE    codigoempresa in (1,2,3,5) \n```\n\n**Relación con `CabeceraAlbaranProveedor`:**\n- `CodigoEmpresa`\n- `EjercicioAlbaran`\n- `SerieAlbaran`\n- `NumeroAlbaran`\n\nNormalmente existe **una cabecera** y **varias líneas** para el mismo albarán.\n\n**Significado de los campos de la consulta:**\n- `CodigoEmpresa`: empresa del grupo a la que pertenece la línea.\n- `EjercicioAlbaran`: ejercicio del albarán.\n- `SerieAlbaran`: serie del albarán.\n- `NumeroAlbaran`: número del albarán.\n- `Orden`: orden interno de la línea dentro del documento.\n- `LineasPosicion`: posición o numeración visual de la línea.\n- `LineaPedido`: línea del pedido de compra relacionada.\n- `FechaRegistro`: fecha de registro de la línea en el sistema.\n- `FechaAlbaran`: fecha del albarán al que pertenece.\n- `CodigoArticulo`: código del artículo recibido.\n- `CodigoAlmacen`: almacén de entrada o imputación.\n- `Partida`: lote, partida o referencia de recepción si aplica.\n- `DescripcionArticulo`: descripción principal del artículo.\n- `Descripcion2Articulo`: segunda descripción o ampliación.\n- `CodigoFamilia`: familia del artículo.\n- `CodigoSubfamilia`: subfamilia del artículo.\n- `CodigodelProveedor`: código del artículo según el proveedor o referencia del proveedor.\n- `CodigoDefinicion_`: código auxiliar de definición o clasificación.\n- `CodigoProyecto`: proyecto asociado a la línea.\n- `CodigoSeccion`: sección asociada a la línea.\n- `CodigoDepartamento`: departamento asociado a la línea.\n- `StatusStock`: indica el tratamiento o estado de stock de la línea.\n- `EjercicioPedido`: ejercicio del pedido relacionado.\n- `SeriePedido`: serie del pedido relacionado.\n- `NumeroPedido`: número del pedido relacionado.\n- `EjercicioFactura`: ejercicio de la factura relacionada.\n- `SerieFactura`: serie de la factura relacionada.\n- `NumeroFactura`: número de la factura relacionada.\n- `Unidades`: cantidad recibida en la línea.\n- `Precio`: precio unitario de compra.\n- `PrecioRebaje`: precio rebajado o ajustado si existe.\n- `BaseImponible`: base imponible de la línea.\n- `CuotaIva`: importe de IVA de la línea.\n- `TotalIva`: total de línea con IVA o importe fiscal total según configuración.\n- `ImporteLiquido`: importe final de la línea.\n- `CAS_PartidaVentas`: referencia interna Castrosua para trazabilidad con ventas, si aplica.\n\n##### `LOGIC_Proveedores`\nContiene los **datos maestros de proveedores**: razón social, CIF/NIF, direcciones, contactos, condiciones de pago, datos bancarios y otros atributos administrativos.\n\n```sql\nSELECT        CodigoEmpresa, CodigoProveedor, CifEuropeo, RazonSocial, RazonSocial2, Nombre,  ObservacionesProveedor, \n                         Comentarios, CodigoContable\nFROM            Proveedores\nWHERE        (LEN(CodigoProveedor) >= 10) AND (CodigoEmpresa IN (1, 2, 3, 5))\n```\n\n**Clave:**\n- `CodigoEmpresa`\n- `CodigoProveedor`\n\n**Relación habitual con albaranes de proveedor:**\n- `CabeceraAlbaranProveedor.CodigoEmpresa = Proveedores.CodigoEmpresa`\n- `CabeceraAlbaranProveedor.CodigoProveedor = Proveedores.CodigoProveedor`\n\n**Significado de los campos de la consulta:**\n- `CodigoEmpresa`: empresa del grupo a la que pertenece el maestro del proveedor.\n- `CodigoProveedor`: código interno del proveedor.\n- `CifEuropeo`: identificación fiscal europea.\n- `RazonSocial`: razón social principal.\n- `RazonSocial2`: ampliación o segunda línea de razón social.\n- `Nombre`: nombre comercial o abreviado.\n- `ObservacionesProveedor`: observaciones generales del proveedor.\n- `Comentarios`: comentarios internos adicionales.\n- `CodigoContable`: cuenta o código contable asociado al proveedor.\n\n##### `LOGIC_Articulos`\nContiene los **datos maestros de artículos**: código, descripción, familia, subfamilia, precios, unidades, stock y otros atributos del artículo.\n\n```sql\nSELECT        CodigoEmpresa, CodigoArticulo, DescripcionArticulo, Descripcion2Articulo,CodigoAlternativo, CodigoAlternativo2,  TipoArticulo, \nPrecioCompra, PrecioVenta,                          CAS_CodigoGruposPlanificacion\nFROM            Articulos\nWHERE        (CodigoEmpresa IN (1))\n```\n\n**Clave:**\n- `CodigoEmpresa`\n- `CodigoArticulo`\n\n**Relación habitual con líneas de albarán:**\n- `LineasAlbaranProveedor.CodigoArticulo = Articulos.CodigoArticulo`\n\n**Significado de los campos de la consulta:**\n- `CodigoEmpresa`: empresa propietaria del maestro de artículo. Para consultas de artículos usar siempre la empresa 1.\n- `CodigoArticulo`: código interno del artículo en Logic.\n- `DescripcionArticulo`: descripción principal del artículo.\n- `Descripcion2Articulo`: segunda descripción o ampliación.\n- `CodigoAlternativo`: referencia alternativa del artículo.\n- `CodigoAlternativo2`: segunda referencia alternativa.\n- `TipoArticulo`: tipo o categoría funcional del artículo.\n- `PrecioCompra`: precio de compra registrado en el maestro.\n- `PrecioVenta`: precio de venta registrado en el maestro.\n- `CAS_CodigoGruposPlanificacion`: código interno Castrosua relacionado con planificación o agrupación.\n\n##### `LOGIC_CabeceraPedidoProveedor`\nGuarda la **cabecera del pedido de proveedor**, es decir, los datos generales del pedido de compra: proveedor, fechas, condiciones, estado, importes, observaciones y vínculos con otros documentos.\n\n**Consulta/Vista:**\n\n```sql\nSELECT       CodigoEmpresa, EjercicioPedido, SeriePedido, NumeroPedido, FechaPedido, CodigoProveedor, NumeroLineas,\n                         RazonSocial, Nombre, Estado, StatusAprobado, ObservacionesProveedor, ObservacionesPedido,\n                         FechaNecesaria, FechaRecepcion, FechaTope,\n                         CodigoProyecto, CodigoSeccion, CodigoDepartamento,\n                         BaseImponible, TotalCuotaIva, ImporteLiquido,\n                         SuPedido, IdPedidoPro, CAS_StatusAnulado, CAS_StatusPedCompras, CAS_PartidaVentas\nFROM            CabeceraPedidoProveedor\nWHERE           CodigoEmpresa IN (1, 2, 3, 5) \n```\n\n**Clave funcional del pedido:**\n- `CodigoEmpresa`\n- `EjercicioPedido`\n- `SeriePedido`\n- `NumeroPedido`\n\n**Significado de los campos de la consulta:**\n- `CodigoEmpresa`: empresa del grupo a la que pertenece el pedido.\n- `EjercicioPedido`: año o ejercicio del pedido.\n- `SeriePedido`: serie documental del pedido.\n- `NumeroPedido`: número interno del pedido.\n- `FechaPedido`: fecha de creación o emisión del pedido.\n- `CodigoProveedor`: código interno del proveedor.\n- `NumeroLineas`: número de líneas del pedido.\n- `RazonSocial`: razón social principal del proveedor.\n- `Nombre`: nombre comercial o abreviado del proveedor.\n- `Estado`: estado general del pedido.\n- `StatusAprobado`: estado de aprobación del pedido.\n- `ObservacionesProveedor`: observaciones generales del proveedor.\n- `ObservacionesPedido`: observaciones propias del pedido.\n- `FechaNecesaria`: fecha en la que se necesita el material.\n- `FechaRecepcion`: fecha prevista o registrada de recepción.\n- `FechaTope`: fecha límite asociada al pedido.\n- `CodigoProyecto`: proyecto asociado.\n- `CodigoSeccion`: sección asociada.\n- `CodigoDepartamento`: departamento asociado.\n- `BaseImponible`: base imponible del pedido.\n- `TotalCuotaIva`: cuota total de IVA.\n- `ImporteLiquido`: importe final líquido del pedido.\n- `SuPedido`: número de pedido del proveedor o referencia externa si aplica.\n- `IdPedidoPro`: identificador interno del pedido.\n- `CAS_StatusAnulado`: indicador de anulación.\n- `CAS_StatusPedCompras`: estado específico del circuito de compras.\n- `CAS_PartidaVentas`: referencia cruzada interna con ventas.\n\n##### `LOGIC_LineasPedidoProveedor`\nGuarda el **detalle de líneas del pedido de proveedor**, es decir, los artículos solicitados, cantidades, fechas, precios, estado de recepción y referencias cruzadas con otros documentos.\n\n**Consulta/Vista:**\n\n```sql\nSELECT        CodigoEmpresa, EjercicioPedido, SeriePedido, NumeroPedido, Orden, LineasPosicion,\n                         FechaRegistro, FechaPedido, CodigoArticulo, CodigoAlmacen, Partida,\n                         DescripcionArticulo, Descripcion2Articulo,\n                         CodigoFamilia, CodigoSubfamilia, CodigodelProveedor,\n                         CodigoProyecto, CodigoSeccion, CodigoDepartamento,\n                         Estado, FechaRecepcion, FechaNecesaria, FechaTope,\n                         UnidadesPedidas, UnidadesRecibidas, UnidadesPendientes,\n                         Precio, BaseImponible, CuotaIva, TotalIva, ImporteLiquido,\n                         CodigoAlternativo, CAS_DescripcionIncidencia, CAS_PartidaVentas\nFROM            LineasPedidoProveedor\nWHERE           CodigoEmpresa IN (1, 2, 3, 5) \n```\n\n**Relación con `CabeceraPedidoProveedor`:**\n- `CodigoEmpresa`\n- `EjercicioPedido`\n- `SeriePedido`\n- `NumeroPedido`\n\nNormalmente existe **una cabecera** y **varias líneas** para el mismo pedido.\n\n**Significado de los campos de la consulta:**\n- `CodigoEmpresa`: empresa del grupo a la que pertenece la línea.\n- `EjercicioPedido`: ejercicio del pedido.\n- `SeriePedido`: serie del pedido.\n- `NumeroPedido`: número del pedido.\n- `Orden`: orden interno de la línea.\n- `LineasPosicion`: posición visible de la línea.\n- `FechaRegistro`: fecha de registro de la línea.\n- `FechaPedido`: fecha del pedido.\n- `CodigoArticulo`: código del artículo solicitado.\n- `CodigoAlmacen`: almacén de destino o recepción.\n- `Partida`: lote, partida o referencia asociada.\n- `DescripcionArticulo`: descripción principal del artículo.\n- `Descripcion2Articulo`: segunda descripción o ampliación.\n- `CodigoFamilia`: familia del artículo.\n- `CodigoSubfamilia`: subfamilia del artículo.\n- `CodigodelProveedor`: referencia del artículo según el proveedor.\n- `CodigoProyecto`: proyecto asociado.\n- `CodigoSeccion`: sección asociada.\n- `CodigoDepartamento`: departamento asociado.\n- `Estado`: estado actual de la línea.\n- `FechaRecepcion`: fecha prevista o efectiva de recepción.\n- `FechaNecesaria`: fecha en la que se necesita el material.\n- `FechaTope`: fecha límite.\n- `UnidadesPedidas`: cantidad pedida.\n- `UnidadesRecibidas`: cantidad ya recibida.\n- `UnidadesPendientes`: cantidad pendiente de recibir.\n- `Precio`: precio unitario.\n- `BaseImponible`: base imponible de la línea.\n- `CuotaIva`: cuota de IVA.\n- `TotalIva`: total con impuestos o impacto fiscal total.\n- `ImporteLiquido`: importe final líquido.\n- `CodigoAlternativo`: referencia alternativa.\n- `CAS_DescripcionIncidencia`: texto de incidencia si existe.\n- `CAS_PartidaVentas`: referencia cruzada con ventas.\n\n##### `LOGIC_ResumenProveedor`\nGuarda la **cabecera de la factura de proveedor**, es decir, el resumen económico y administrativo de la factura de compra.\n\n**Consulta/Vista:**\n\n```sql\nSELECT        CodigoEmpresa, EjercicioFactura, SerieFactura, NumeroFactura, FechaFactura, FechaSuFactura, CodigoProveedor,\n                         RazonSocial, Nombre, StatusAbono, StatusFacturaListada, StatusCartera, StatusContabilidad,\n                         ObservacionesProveedor, ObservacionesFactura,\n                         CodigoProyecto, CodigoSeccion, CodigoDepartamento,\n                         BaseImponible, TotalCuotaIva, TotalIva, ImporteFactura, ImporteLiquido,\n                         SuFacturaNo, IdFacturaPro, FechaOperacion, FechaMaxVencimiento, FechaLiquidacion,\n                         CAS_FechaEntradaFactura, CAS_FechaValidacion\nFROM            ResumenProveedor\nWHERE           CodigoEmpresa IN (1, 2, 3, 5)\n```\n\n**Clave funcional de la factura:**\n- `CodigoEmpresa`\n- `EjercicioFactura`\n- `SerieFactura`\n- `NumeroFactura`\n\n**Relación habitual:**\n- `LineasAlbaranProveedor.CodigoEmpresa = ResumenProveedor.CodigoEmpresa`\n- `LineasAlbaranProveedor.EjercicioFactura = ResumenProveedor.EjercicioFactura`\n- `LineasAlbaranProveedor.SerieFactura = ResumenProveedor.SerieFactura`\n- `LineasAlbaranProveedor.NumeroFactura = ResumenProveedor.NumeroFactura`\n- `ResumenProveedor.CodigoEmpresa = Proveedores.CodigoEmpresa`\n- `ResumenProveedor.CodigoProveedor = Proveedores.CodigoProveedor`\n\n**Significado de los campos de la consulta:**\n- `CodigoEmpresa`: empresa del grupo a la que pertenece la factura.\n- `EjercicioFactura`: año o ejercicio de la factura.\n- `SerieFactura`: serie documental de la factura.\n- `NumeroFactura`: número interno de la factura.\n- `FechaFactura`: fecha de la factura en el ERP.\n- `FechaSuFactura`: fecha de la factura del proveedor.\n- `CodigoProveedor`: código interno del proveedor.\n- `RazonSocial`: razón social principal del proveedor.\n- `Nombre`: nombre comercial o abreviado del proveedor.\n- `StatusAbono`: indica si la factura es de abono o su estado relacionado.\n- `StatusFacturaListada`: estado de listado o emisión documental.\n- `StatusCartera`: estado en cartera o pagos.\n- `StatusContabilidad`: estado contable de la factura.\n- `ObservacionesProveedor`: observaciones generales del proveedor.\n- `ObservacionesFactura`: observaciones propias de la factura.\n- `CodigoProyecto`: proyecto asociado.\n- `CodigoSeccion`: sección asociada.\n- `CodigoDepartamento`: departamento asociado.\n- `BaseImponible`: base imponible de la factura.\n- `TotalCuotaIva`: cuota total de IVA.\n- `TotalIva`: total con impuestos.\n- `ImporteFactura`: importe de la factura.\n- `ImporteLiquido`: importe final líquido.\n- `SuFacturaNo`: número de factura del proveedor.\n- `IdFacturaPro`: identificador interno de la factura.\n- `FechaOperacion`: fecha de operación.\n- `FechaMaxVencimiento`: fecha máxima de vencimiento.\n- `FechaLiquidacion`: fecha de liquidación.\n- `CAS_FechaEntradaFactura`: fecha de entrada de la factura.\n- `CAS_FechaValidacion`: fecha de validación.\n\n##### `LOGIC_Clientes`\nContiene los **datos maestros de clientes**: razón social, CIF/NIF, direcciones, contactos, condiciones comerciales y otros atributos administrativos.\n\n**Consulta/Vista:**\n\n```sql\nSELECT       CodigoEmpresa, CodigoCliente, CifEuropeo, RazonSocial, RazonSocial2, Nombre,\n                         ObservacionesCliente, Comentarios, CodigoContable\nFROM            Clientes\nWHERE        (CodigoEmpresa IN (1, 2, 3, 5)) AND (CodigoCategoriaCliente_ = 'CLI') AND (LEN(CodigoCliente) > 10)\n```\n\n**Clave:**\n- `CodigoEmpresa`\n- `CodigoCliente`\n\n**Relación habitual con albaranes y pedidos de cliente:**\n- `CabeceraAlbaranCliente.CodigoEmpresa = Clientes.CodigoEmpresa`\n- `CabeceraAlbaranCliente.CodigoCliente = Clientes.CodigoCliente`\n- `CabeceraPedidoCliente.CodigoEmpresa = Clientes.CodigoEmpresa`\n- `CabeceraPedidoCliente.CodigoCliente = Clientes.CodigoCliente`\n\n**Significado de los campos de la consulta:**\n- `CodigoEmpresa`: empresa del grupo a la que pertenece el maestro del cliente.\n- `CodigoCliente`: código interno del cliente.\n- `CifEuropeo`: identificación fiscal europea.\n- `RazonSocial`: razón social principal.\n- `RazonSocial2`: ampliación o segunda línea de razón social.\n- `Nombre`: nombre comercial o abreviado.\n- `ObservacionesCliente`: observaciones generales del cliente.\n- `Comentarios`: comentarios internos adicionales.\n- `CodigoContable`: cuenta o código contable asociado al cliente.\n\n##### `LOGIC_CabeceraAlbaranCliente`\nGuarda la **cabecera del albarán de cliente**, es decir, los datos generales del documento de entrega o venta: cliente, fechas, importes, estados y observaciones.\n\n**Consulta/Vista:**\n\n```sql\nSELECT        CodigoEmpresa, EjercicioAlbaran, SerieAlbaran, NumeroAlbaran, FechaAlbaran, CodigoCliente, NumeroLineas,\n                         RazonSocial, Nombre, StatusFacturado,\n                         CodigoProyecto, CodigoSeccion, CodigoDepartamento,\n                         ObservacionesCliente, ObservacionesAlbaran, ObservacionesFactura,\n                         SuPedido, EjercicioPedido, SeriePedido, NumeroPedido,\n                         FechaFactura, EjercicioFactura, SerieFactura, NumeroFactura,\n                         BaseImponible, TotalCuotaIva, ImporteLiquido,\n                         NoFacturable, IdAlbaranCli, CAS_FechaEntrega, SuContrato\nFROM            CabeceraAlbaranCliente\nWHERE           CodigoEmpresa IN (1, 2, 3, 5)\n```\n\n**Clave funcional del albarán:**\n- `CodigoEmpresa`\n- `EjercicioAlbaran`\n- `SerieAlbaran`\n- `NumeroAlbaran`\n\n**Significado de los campos de la consulta:**\n- `CodigoEmpresa`: empresa del grupo a la que pertenece el albarán.\n- `EjercicioAlbaran`: año o ejercicio del albarán.\n- `SerieAlbaran`: serie documental del albarán.\n- `NumeroAlbaran`: número interno del albarán.\n- `FechaAlbaran`: fecha del albarán.\n- `CodigoCliente`: código interno del cliente.\n- `NumeroLineas`: número de líneas del documento.\n- `RazonSocial`: razón social principal del cliente.\n- `Nombre`: nombre comercial o abreviado del cliente.\n- `StatusFacturado`: indica si el albarán está facturado o su estado de facturación.\n- `CodigoProyecto`: proyecto asociado.\n- `CodigoSeccion`: sección asociada.\n- `CodigoDepartamento`: departamento asociado.\n- `ObservacionesCliente`: observaciones generales del cliente.\n- `ObservacionesAlbaran`: observaciones propias del albarán.\n- `ObservacionesFactura`: observaciones relacionadas con la factura.\n- `SuPedido`: número de pedido del cliente o referencia externa.\n- `EjercicioPedido`: ejercicio del pedido relacionado.\n- `SeriePedido`: serie del pedido relacionado.\n- `NumeroPedido`: número del pedido relacionado.\n- `FechaFactura`: fecha de la factura asociada.\n- `EjercicioFactura`: ejercicio de la factura asociada.\n- `SerieFactura`: serie de la factura asociada.\n- `NumeroFactura`: número de la factura asociada.\n- `BaseImponible`: base imponible del albarán.\n- `TotalCuotaIva`: cuota total de IVA.\n- `ImporteLiquido`: importe final líquido del albarán.\n- `NoFacturable`: marca de no facturable.\n- `IdAlbaranCli`: identificador interno del albarán.\n- `CAS_FechaEntrega`: fecha de entrega.\n- `SuContrato`: referencia de contrato si existe.\n\n##### `LOGIC_LineasAlbaranCliente`\nGuarda el **detalle de líneas del albarán de cliente**, es decir, los artículos servidos o entregados, cantidades, precios, impuestos e importes por línea.\n\n**Consulta/Vista:**\n\n```sql\nSELECT        CodigoEmpresa, EjercicioAlbaran, SerieAlbaran, NumeroAlbaran, Orden, LineasPosicion,\n                         FechaRegistro, FechaAlbaran, CodigoArticulo, CodigoAlmacen, Partida,\n                         DescripcionArticulo, Descripcion2Articulo,\n                         CodigoFamilia, CodigoSubfamilia, CodigodelCliente,\n                         CodigoProyecto, CodigoSeccion, CodigoDepartamento,\n                         StatusStock, EjercicioPedido, SeriePedido, NumeroPedido,\n                         EjercicioFactura, SerieFactura, NumeroFactura,\n                         Unidades, Precio, PrecioRebaje, BaseImponible, CuotaIva, TotalIva, ImporteLiquido,\n                         NumeroSerieLc, CAS_NumeroBastidor, CAS_PartidaVentas\nFROM            LineasAlbaranCliente\nWHERE           CodigoEmpresa IN (1, 2, 3, 5)\n```\n\n**Relación con `CabeceraAlbaranCliente`:**\n- `CodigoEmpresa`\n- `EjercicioAlbaran`\n- `SerieAlbaran`\n- `NumeroAlbaran`\n\nNormalmente existe **una cabecera** y **varias líneas** para el mismo albarán.\n\n**Significado de los campos de la consulta:**\n- `CodigoEmpresa`: empresa del grupo a la que pertenece la línea.\n- `EjercicioAlbaran`: ejercicio del albarán.\n- `SerieAlbaran`: serie del albarán.\n- `NumeroAlbaran`: número del albarán.\n- `Orden`: orden interno de la línea.\n- `LineasPosicion`: posición visible de la línea.\n- `FechaRegistro`: fecha de registro de la línea.\n- `FechaAlbaran`: fecha del albarán.\n- `CodigoArticulo`: código del artículo servido.\n- `CodigoAlmacen`: almacén de salida o imputación.\n- `Partida`: lote, partida o referencia asociada.\n- `DescripcionArticulo`: descripción principal del artículo.\n- `Descripcion2Articulo`: segunda descripción o ampliación.\n- `CodigoFamilia`: familia del artículo.\n- `CodigoSubfamilia`: subfamilia del artículo.\n- `CodigodelCliente`: referencia del artículo según el cliente.\n- `CodigoProyecto`: proyecto asociado.\n- `CodigoSeccion`: sección asociada.\n- `CodigoDepartamento`: departamento asociado.\n- `StatusStock`: tratamiento o estado de stock de la línea.\n- `EjercicioPedido`: ejercicio del pedido relacionado.\n- `SeriePedido`: serie del pedido relacionado.\n- `NumeroPedido`: número del pedido relacionado.\n- `EjercicioFactura`: ejercicio de la factura relacionada.\n- `SerieFactura`: serie de la factura relacionada.\n- `NumeroFactura`: número de la factura relacionada.\n- `Unidades`: cantidad servida.\n- `Precio`: precio unitario.\n- `PrecioRebaje`: precio rebajado o ajustado.\n- `BaseImponible`: base imponible de la línea.\n- `CuotaIva`: cuota de IVA.\n- `TotalIva`: total con impuestos o impacto fiscal total.\n- `ImporteLiquido`: importe final líquido.\n- `NumeroSerieLc`: número de serie si aplica.\n- `CAS_NumeroBastidor`: número de bastidor si aplica.\n- `CAS_PartidaVentas`: referencia cruzada con ventas.\n\n##### `LOGIC_ResumenCliente`\nGuarda la **cabecera de la factura de cliente**, es decir, el resumen económico y administrativo de la factura de venta.\n\n**Consulta/Vista:**\n\n```sql\nSELECT        CodigoEmpresa, EjercicioFactura, SerieFactura, NumeroFactura, FechaFactura, CodigoCliente,\n                         RazonSocial, Nombre, StatusAbono, StatusFacturaListada, StatusCartera, StatusContabilidad,\n                         ObservacionesCliente, ObservacionesFactura,\n                         CodigoProyecto, CodigoSeccion, CodigoDepartamento,\n                         BaseImponible, TotalCuotaIva, TotalIva, ImporteFactura, ImporteLiquido,\n                         EjercicioFacturaOriginal, SerieFacturaOriginal, NumeroFacturaOriginal,\n                         FechaOperacion, IdFacturaCli, FechaMaxVencimiento, FechaLiquidacion, SerieAFacturar\nFROM            ResumenCliente\nWHERE           CodigoEmpresa IN (1, 2, 3, 5)\n```\n\n**Clave funcional de la factura:**\n- `CodigoEmpresa`\n- `EjercicioFactura`\n- `SerieFactura`\n- `NumeroFactura`\n\n**Relación habitual:**\n- `LineasAlbaranCliente.CodigoEmpresa = ResumenCliente.CodigoEmpresa`\n- `LineasAlbaranCliente.EjercicioFactura = ResumenCliente.EjercicioFactura`\n- `LineasAlbaranCliente.SerieFactura = ResumenCliente.SerieFactura`\n- `LineasAlbaranCliente.NumeroFactura = ResumenCliente.NumeroFactura`\n- `ResumenCliente.CodigoEmpresa = Clientes.CodigoEmpresa`\n- `ResumenCliente.CodigoCliente = Clientes.CodigoCliente`\n\n**Significado de los campos de la consulta:**\n- `CodigoEmpresa`: empresa del grupo a la que pertenece la factura.\n- `EjercicioFactura`: año o ejercicio de la factura.\n- `SerieFactura`: serie documental de la factura.\n- `NumeroFactura`: número interno de la factura.\n- `FechaFactura`: fecha de la factura.\n- `CodigoCliente`: código interno del cliente.\n- `RazonSocial`: razón social principal del cliente.\n- `Nombre`: nombre comercial o abreviado del cliente.\n- `StatusAbono`: indica si la factura es rectificativa o de abono, o su estado relacionado.\n- `StatusFacturaListada`: estado de listado o emisión documental.\n- `StatusCartera`: estado en cartera o cobros.\n- `StatusContabilidad`: estado contable de la factura.\n- `ObservacionesCliente`: observaciones generales del cliente.\n- `ObservacionesFactura`: observaciones propias de la factura.\n- `CodigoProyecto`: proyecto asociado.\n- `CodigoSeccion`: sección asociada.\n- `CodigoDepartamento`: departamento asociado.\n- `BaseImponible`: base imponible de la factura.\n- `TotalCuotaIva`: cuota total de IVA.\n- `TotalIva`: total con impuestos.\n- `ImporteFactura`: importe de la factura.\n- `ImporteLiquido`: importe final líquido.\n- `EjercicioFacturaOriginal`: ejercicio de la factura original si es rectificativa.\n- `SerieFacturaOriginal`: serie de la factura original.\n- `NumeroFacturaOriginal`: número de la factura original.\n- `FechaOperacion`: fecha de operación.\n- `IdFacturaCli`: identificador interno de la factura.\n- `FechaMaxVencimiento`: fecha máxima de vencimiento.\n- `FechaLiquidacion`: fecha de liquidación.\n- `SerieAFacturar`: serie comercial aplicada o prevista de facturación.\n\n##### `LOGIC_CabeceraPedidoCliente`\nGuarda la **cabecera del pedido de cliente**, es decir, los datos generales del pedido de venta: cliente, fechas, estado, importes, observaciones y referencias documentales.\n\n**Consulta/Vista:**\n\n```sql\nSELECT         CodigoEmpresa, EjercicioPedido, SeriePedido, NumeroPedido, FechaPedido, CodigoCliente, NumeroLineas,\n                         RazonSocial, Nombre, Estado, StatusAprobado,\n                         ObservacionesCliente, ObservacionesPedido,\n                         FechaNecesaria, FechaEntrega, FechaTope,\n                         CodigoProyecto, CodigoSeccion, CodigoDepartamento,\n                         BaseImponible, TotalCuotaIva, ImporteLiquido,\n                         SuPedido, IdPedidoCli, NoFacturable, SuContrato, StatusWF\nFROM            CabeceraPedidoCliente\nWHERE           CodigoEmpresa IN (1, 2, 3, 5)\n```\n\n**Clave funcional del pedido:**\n- `CodigoEmpresa`\n- `EjercicioPedido`\n- `SeriePedido`\n- `NumeroPedido`\n\n**Significado de los campos de la consulta:**\n- `CodigoEmpresa`: empresa del grupo a la que pertenece el pedido.\n- `EjercicioPedido`: año o ejercicio del pedido.\n- `SeriePedido`: serie documental del pedido.\n- `NumeroPedido`: número interno del pedido.\n- `FechaPedido`: fecha de creación o emisión del pedido.\n- `CodigoCliente`: código interno del cliente.\n- `NumeroLineas`: número de líneas del pedido.\n- `RazonSocial`: razón social principal del cliente.\n- `Nombre`: nombre comercial o abreviado del cliente.\n- `Estado`: estado general del pedido.\n- `StatusAprobado`: estado de aprobación del pedido.\n- `ObservacionesCliente`: observaciones generales del cliente.\n- `ObservacionesPedido`: observaciones propias del pedido.\n- `FechaNecesaria`: fecha en la que se necesita servir el material.\n- `FechaEntrega`: fecha prevista de entrega.\n- `FechaTope`: fecha límite asociada al pedido.\n- `CodigoProyecto`: proyecto asociado.\n- `CodigoSeccion`: sección asociada.\n- `CodigoDepartamento`: departamento asociado.\n- `BaseImponible`: base imponible del pedido.\n- `TotalCuotaIva`: cuota total de IVA.\n- `ImporteLiquido`: importe final líquido del pedido.\n- `SuPedido`: número de pedido del cliente o referencia externa.\n- `IdPedidoCli`: identificador interno del pedido.\n- `NoFacturable`: marca de no facturable.\n- `SuContrato`: referencia de contrato si existe.\n- `StatusWF`: estado del flujo de trabajo.\n\n##### `LOGIC_LineasPedidoCliente`\nGuarda el **detalle de líneas del pedido de cliente**, es decir, los artículos solicitados, cantidades, fechas, precios, estado de servicio y referencias cruzadas.\n\n**Consulta/Vista:**\n\n```sql\nSELECT         CodigoEmpresa, EjercicioPedido, SeriePedido, NumeroPedido, Orden, LineasPosicion,\n                         FechaRegistro, FechaPedido, CodigoArticulo, CodigoAlmacen, Partida,\n                         DescripcionArticulo, Descripcion2Articulo,\n                         CodigoFamilia, CodigoSubfamilia, CodigodelCliente,\n                         CodigoProyecto, CodigoSeccion, CodigoDepartamento,\n                         FechaNecesaria, FechaEntrega, FechaTope,\n                         Estado, EjercicioPedidoPro, SeriePedidoPro, NumeroPedidoPro,\n                         UnidadesPedidas, UnidadesServidas, UnidadesPendientes,\n                         Precio, BaseImponible, CuotaIva, TotalIva, ImporteLiquido,\n                         NumeroSerieLc, CAS_NumeroBastidor, CAS_PartidaVentas\nFROM            LineasPedidoCliente\nWHERE           CodigoEmpresa IN (1, 2, 3, 5)\n```\n\n**Relación con `CabeceraPedidoCliente`:**\n- `CodigoEmpresa`\n- `EjercicioPedido`\n- `SeriePedido`\n- `NumeroPedido`\n\nNormalmente existe **una cabecera** y **varias líneas** para el mismo pedido.\n\n**Significado de los campos de la consulta:**\n- `CodigoEmpresa`: empresa del grupo a la que pertenece la línea.\n- `EjercicioPedido`: ejercicio del pedido.\n- `SeriePedido`: serie del pedido.\n- `NumeroPedido`: número del pedido.\n- `Orden`: orden interno de la línea.\n- `LineasPosicion`: posición visible de la línea.\n- `FechaRegistro`: fecha de registro de la línea.\n- `FechaPedido`: fecha del pedido.\n- `CodigoArticulo`: código del artículo solicitado.\n- `CodigoAlmacen`: almacén de servicio o imputación.\n- `Partida`: lote, partida o referencia asociada.\n- `DescripcionArticulo`: descripción principal del artículo.\n- `Descripcion2Articulo`: segunda descripción o ampliación.\n- `CodigoFamilia`: familia del artículo.\n- `CodigoSubfamilia`: subfamilia del artículo.\n- `CodigodelCliente`: referencia del artículo según el cliente.\n- `CodigoProyecto`: proyecto asociado.\n- `CodigoSeccion`: sección asociada.\n- `CodigoDepartamento`: departamento asociado.\n- `FechaNecesaria`: fecha en la que se necesita el material.\n- `FechaEntrega`: fecha prevista de entrega.\n- `FechaTope`: fecha límite.\n- `Estado`: estado actual de la línea.\n- `EjercicioPedidoPro`: ejercicio del pedido de proveedor relacionado si existe.\n- `SeriePedidoPro`: serie del pedido de proveedor relacionado.\n- `NumeroPedidoPro`: número del pedido de proveedor relacionado.\n- `UnidadesPedidas`: cantidad pedida.\n- `UnidadesServidas`: cantidad ya servida.\n- `UnidadesPendientes`: cantidad pendiente de servir.\n- `Precio`: precio unitario.\n- `BaseImponible`: base imponible de la línea.\n- `CuotaIva`: cuota de IVA.\n- `TotalIva`: total con impuestos o impacto fiscal total.\n- `ImporteLiquido`: importe final líquido.\n- `NumeroSerieLc`: número de serie si aplica.\n- `CAS_NumeroBastidor`: número de bastidor si aplica.\n- `CAS_PartidaVentas`: referencia cruzada interna de ventas.\n\n##### `LOGIC_AcumuladoStock`\nGuarda los **acumulados de stock por artículo, almacén, ejercicio y periodo**: entradas, salidas, saldo, importes y precios medios.\n\n**Consulta/Vista:**\n\n```sql\nSELECT         CodigoEmpresa, Ejercicio, CodigoArticulo, CodigoAlmacen, Periodo,  UnidadEntrada, UnidadSalida, UnidadSaldo,  ImporteEntrada, CosteSalida, ImporteSaldo, ImporteSalida,  PrecioMedio, \n                         PrecioUltimaEntrada, PrecioUltimaSalida, FechaUltimaEntrada, FechaUltimaSalida\nFROM            AcumuladoStock\nWHERE           CodigoEmpresa IN (1)\n```\n\n**Clave funcional del acumulado:**\n- `CodigoEmpresa`\n- `Ejercicio`\n- `CodigoArticulo`\n- `CodigoAlmacen`\n- `Periodo`\n\n**Interpretación importante del periodo:**\n- `Periodo = 0`: apertura de ejercicio.\n- `Periodo = 99`: resumen acumulado del año.\n- `Periodo = 1` a `12`: meses aislados.\n\n**Regla importante:**\n- Esta vista debe consultarse solo con `CodigoEmpresa = 1`.\n- Los artículos se interpretan igual que en `Articulos`, usando empresa 1 como referencia general.\n\n**Significado de los campos de la consulta:**\n- `CodigoEmpresa`: empresa del acumulado. Usar siempre 1.\n- `Ejercicio`: año o ejercicio del acumulado.\n- `CodigoArticulo`: código del artículo.\n- `CodigoAlmacen`: almacén al que corresponde el acumulado.\n- `Periodo`: periodo contable o mensual del acumulado.\n- `UnidadEntrada`: unidades entradas en el periodo.\n- `UnidadSalida`: unidades salidas en el periodo.\n- `UnidadSaldo`: saldo de unidades al cierre del periodo.\n- `ImporteEntrada`: importe de las entradas del periodo.\n- `CosteSalida`: coste asociado a las salidas del periodo.\n- `ImporteSaldo`: valoración del saldo al cierre del periodo.\n- `ImporteSalida`: importe de las salidas del periodo.\n- `PrecioMedio`: precio medio calculado del artículo en ese periodo.\n- `PrecioUltimaEntrada`: precio de la última entrada registrada.\n- `PrecioUltimaSalida`: precio o coste de la última salida registrada.\n- `FechaUltimaEntrada`: fecha de la última entrada.\n- `FechaUltimaSalida`: fecha de la última salida.\n\n#### Regla importante para artículos\nAunque la tabla `Articulos` incluye `CodigoEmpresa`, **solo se deben tener en cuenta los artículos de la empresa 1**, porque sus artículos **aplican a todas las empresas**.\n\nPor tanto, al relacionar artículos:\n- usar `Articulos.CodigoEmpresa = 1`\n- relacionar por `LineasAlbaranProveedor.CodigoArticulo = Articulos.CodigoArticulo`\n- relacionar por `LineasPedidoProveedor.CodigoArticulo = Articulos.CodigoArticulo`\n- relacionar por `LineasAlbaranCliente.CodigoArticulo = Articulos.CodigoArticulo`\n- relacionar por `LineasPedidoCliente.CodigoArticulo = Articulos.CodigoArticulo`\n- relacionar por `AcumuladoStock.CodigoArticulo = Articulos.CodigoArticulo`\n- en búsquedas directas de códigos de artículo, consultar también con `Articulos.CodigoEmpresa = 1`\n\n#### Resumen de relaciones\n- **Cabecera ↔ Líneas de albarán de proveedor** por:\n  - `CodigoEmpresa`\n  - `EjercicioAlbaran`\n  - `SerieAlbaran`\n  - `NumeroAlbaran`\n\n- **Cabecera de albarán proveedor ↔ Proveedores** por:\n  - `CodigoEmpresa`\n  - `CodigoProveedor`\n\n- **Líneas de albarán proveedor ↔ ResumenProveedor** por:\n  - `CodigoEmpresa`\n  - `EjercicioFactura`\n  - `SerieFactura`\n  - `NumeroFactura`\n\n- **ResumenProveedor ↔ Proveedores** por:\n  - `CodigoEmpresa`\n  - `CodigoProveedor`\n\n- **Cabecera de pedido proveedor ↔ Líneas de pedido proveedor** por:\n  - `CodigoEmpresa`\n  - `EjercicioPedido`\n  - `SeriePedido`\n  - `NumeroPedido`\n\n- **Cabecera de pedido proveedor ↔ Proveedores** por:\n  - `CodigoEmpresa`\n  - `CodigoProveedor`\n\n- **Cabecera ↔ Líneas de albarán de cliente** por:\n  - `CodigoEmpresa`\n  - `EjercicioAlbaran`\n  - `SerieAlbaran`\n  - `NumeroAlbaran`\n\n- **Cabecera de albarán cliente ↔ Clientes** por:\n  - `CodigoEmpresa`\n  - `CodigoCliente`\n\n- **Líneas de albarán cliente ↔ ResumenCliente** por:\n  - `CodigoEmpresa`\n  - `EjercicioFactura`\n  - `SerieFactura`\n  - `NumeroFactura`\n\n- **ResumenCliente ↔ Clientes** por:\n  - `CodigoEmpresa`\n  - `CodigoCliente`\n\n- **Cabecera de pedido cliente ↔ Líneas de pedido cliente** por:\n  - `CodigoEmpresa`\n  - `EjercicioPedido`\n  - `SeriePedido`\n  - `NumeroPedido`\n\n- **Cabecera de pedido cliente ↔ Clientes** por:\n  - `CodigoEmpresa`\n  - `CodigoCliente`\n\n- **Líneas de proveedor/cliente ↔ Artículos** por:\n  - `CodigoArticulo`\n  - filtrando siempre `Articulos.CodigoEmpresa = 1`\n\n- **AcumuladoStock ↔ Artículos** por:\n  - `CodigoArticulo`\n  - filtrando siempre `Articulos.CodigoEmpresa = 1`\n\n---\n\n### `GLP4`\nUsar esta base de datos cuando el usuario pregunte por:\n- **APPi**\n\n> Pendiente de descripción funcional y técnica. Se completará más adelante.\n\n---\n\n## Instrucción general para la IA\nAntes de responder o construir una consulta, la IA debe identificar primero **qué sistema menciona el usuario**:\n\n- Si habla de **APPi**, consultar **GLP4**.\n- Si habla de **Sage**, **Logic** o **ERP**, consultar la base de datos **ERP Logic / Sage**.\n- Si aporta un **código numérico de 6 dígitos** en contexto de materiales o documentación técnica, interpretarlo por defecto como **`CodigoArticulo` de Logic** y consultar **ERP Logic / Sage**.\n\nSi en el futuro se añaden más sistemas o bases de datos, deberán incorporarse como nuevos bloques de este mismo supernivel.\n\n# 02/06/2026 Nuevas vistas\n";
...
const disableGlobalSqlTool = process.env.${OFFICE_AGENT_DISABLE_GLOBAL_SQL_TOOL_ENV_NAME} === "1";
...
export default function (pi: ExtensionAPI) {
  if (disableGlobalSqlTool) return;
...
export function getOfficeAgentManagedSessionEnv(
  ...
  return {
    ...
    ...(options.activeProjectDir
      ? {
          [OFFICE_AGENT_WORKSPACE_ENV_NAME]: options.activeProjectDir,
          [OFFICE_AGENT_TOOL_FILES_ENV_NAME]: path.join(options.activeProjectDir, OFFICE_AGENT_TOOL_FILES_DIR_NAME),
        }
      : {}),
