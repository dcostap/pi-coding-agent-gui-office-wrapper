...
```txt
virtual://server_iso_docs/
```

Example agent calls:

```txt
read path="virtual://server_iso_docs/policies/quality.md"
ls path="virtual://server_iso_docs"
find pattern="**/*.md" path="virtual://server_iso_docs"
grep pattern="risk assessment" path="virtual://server_iso_docs" ignoreCase=true
```

The client maps `virtual://server_iso_docs` to backend `rootId: "iso_docs"`; the backend maps that root ID to a configured Linux directory, for example:

```txt
virtual://server_iso_docs -> iso_docs -> /srv/officeagent/vfs/iso_docs
```
...
```bash
rg foo virtual://server_iso_docs
type virtual://server_iso_docs\file.txt
cat virtual://server_iso_docs/file.txt
```
...
1. Use normal `read`, `ls`, `find`, and `grep` for `virtual://server_iso_docs`.
2. Use normal tools for local workspace paths.
3. `virtual://server_iso_docs` is read-only.
4. `edit` and `write` to `virtual://server_iso_docs` are blocked.
5. `bash` cannot access `virtual://server_iso_docs` as a remote folder because it is not mounted or mirrored.
...
```txt
virtual://server_iso_docs/policies/quality.md:42: risk assessment must be reviewed annually
virtual://server_iso_docs/procedures/audit.md:18: internal risk assessment evidence
```
...
```txt
virtual://server_iso_docs/path/to/file.md
```
...
- It avoids collisions with real workspace folders such as `server_iso_docs/`.
...
```txt
virtual:server_iso_docs/path/to/file.md
vfs://server_iso_docs/path/to/file.md
officeagent-vfs://server_iso_docs/path/to/file.md
```

`virtual://server_iso_docs/...` is the preferred MVP format because it is self-explanatory while still URI-like. `vfs://...` is shorter but more jargon-heavy; `officeagent-vfs://...` is explicit but too long for frequent agent use.

### Prefix registry

Start with a static client registry:
...
Later this can become server-provided configuration, but static client config is simpler for MVP. The backend root path should be configured server-side, preferably by environment, for example:

```txt
OFFICE_AGENT_VFS_ROOT_ISO_DOCS=/srv/officeagent/vfs/iso_docs
```
...
virtual://server_iso_docs
virtual://server_iso_docs/
virtual://server_iso_docs/foo.md
virtual://server_iso_docs/folder/file.md
```
...
```txt
./virtual://server_iso_docs/foo.md
%OFFICE_AGENT_WORKSPACE%\virtual://server_iso_docs\foo.md
../virtual://server_iso_docs/foo.md
```
...
For MVP, virtual URI components are exact lowercase identifiers. Treat `virtual://server_iso_docs` as virtual; do not silently accept case variants such as `Virtual://server_iso_docs` or `virtual://Server_Iso_Docs`, even on Windows. This avoids surprising aliases and keeps server root IDs stable.

Client-side parser must reject malformed virtual paths before making backend requests, even though the backend must repeat all safety checks. Reject NUL bytes, unsupported schemes, empty authorities, unknown authorities, drive-letter fragments inside virtual paths, UNC-looking paths, backslash path separators, and traversal that would escape the virtual root. Use explicit `path.win32`-style checks for drive letters/UNC/backslashes in tests so behavior is stable even when tests run cross-platform.
...
Because `bash` is not virtualized, commands that mention `virtual://server_iso_docs` will not access remote content. For MVP, do not try to parse and rewrite shell commands. Instead, wrap the OfficeAgent `bash` tool so that if the command string contains a configured virtual URI prefix, the tool result includes an appended advisory note, for example:

```txt
NOTE: virtual://server_iso_docs is an OfficeAgent virtual filesystem URI, not a local folder. Bash cannot access it. Use read, ls, find, or grep with that virtual path instead.
```
...
```txt
virtual://server_iso_docs/policies/quality.md
```
...
```txt
/srv/officeagent/vfs/iso_docs/policies/quality.md
```
...
Binary or non-UTF-8 server files should fail clearly for MVP, for example `virtual://server_iso_docs/path is not a UTF-8 text file`, rather than returning mojibake.
...
- `ls path="virtual://server_iso_docs"` lists backend root.
...
Remote results must include the virtual prefix and preserve the searched subpath. Searching `virtual://server_iso_docs/procedures` should return:

```txt
virtual://server_iso_docs/procedures/audit.md
virtual://server_iso_docs/procedures/internal/review.md
```
...
Do not automatically include remote roots when `path` is omitted or `path="."`. Require explicit `path="virtual://server_iso_docs"` to avoid surprising/expensive remote searches.
...
```txt
virtual://server_iso_docs/path/file.md:123: matched text
virtual://server_iso_docs/path/file.md-124- context text
```
...
```txt
virtual://server_iso_docs is read-only remote content. Use read, ls, find, or grep for virtual://server_iso_docs paths.
```
...
```txt
Virtual roots such as virtual://server_iso_docs are not real local folders. Bash commands cannot access them as remote content. Use read, ls, find, and grep for virtual roots.
```

Additionally, wrap the OfficeAgent `bash` tool so that when `command` contains a configured virtual URI prefix such as `virtual://server_iso_docs`, the final tool result includes an advisory note:
...
A static root registry is enough initially; `GET /v1/vfs/roots` can be added later if the client should discover roots dynamically.
...
1. Backend owns `rootId -> uriPrefix` and returns fully Pi-like payloads with paths such as `virtual://server_iso_docs/...`.
...
Recommended MVP: backend returns structured results, and the OfficeAgent client formats final Pi-compatible tool text. This keeps the backend independent from Pi rendering conventions and avoids coupling it to client-side virtual prefix names. The client already knows `virtual://server_iso_docs -> iso_docs`.
...
```txt
virtual://server_iso_docs/policies/quality.md:42: risk assessment...
```
...
- All returned paths must be virtual paths such as `virtual://server_iso_docs/...`, never real server paths.
...
```json
{
  ...
  "error": {
    "code": "not_found",
    "message": "Path not found: virtual://server_iso_docs/foo.md"
  }
}
```
...
- virtual://server_iso_docs/: company ISO/legal documentation hosted on the OfficeAgent server.

Use read, ls, find, and grep with paths under virtual://server_iso_docs to inspect this content.
Virtual folders are not real local folders: bash cannot access them as remote content, and edit/write are blocked.
When find or grep returns virtual://server_iso_docs/... paths, use those exact paths in follow-up read calls.
Do not search virtual://server_iso_docs unless the task calls for company documentation or the user asks for it.
```
...
- virtual root config type
- default root config for `virtual://server_iso_docs -> iso_docs`
...
- map `iso_docs` to the configured Linux folder, for example from `OFFICE_AGENT_VFS_ROOT_ISO_DOCS`
- run `rg` for grep
...
- `virtual://server_iso_docs/foo.md` routes to `{ rootId: "iso_docs", path: "/foo.md" }`
- `virtual://server_iso_docs` and `virtual://server_iso_docs/` route to `{ rootId: "iso_docs", path: "/" }`
- `./virtual://server_iso_docs/foo.md` is rejected or treated as a normal malformed local path; it must not route remotely
- `%OFFICE_AGENT_WORKSPACE%\virtual://server_iso_docs\foo.md` is rejected; virtual URIs are not workspace paths
- backslash virtual paths such as `virtual://server_iso_docs\foo.md` are rejected; use URI `/` separators
- case variants such as `Virtual://server_iso_docs` and `virtual://Server_Iso_Docs` are rejected
...
- `ls .` includes configured virtual URI roots such as `virtual://server_iso_docs/`
...
- `write virtual://server_iso_docs/foo.md` fails before creating local files
- `edit virtual://server_iso_docs/foo.md` fails before local access/write
...
- `bash` appends the virtual URI advisory note when its command mentions `virtual://server_iso_docs`
...
- [ ] Add virtual root config for `virtual://server_iso_docs -> iso_docs`.
...
- [ ] Ensure client-formatted remote `find`/`grep` outputs include `virtual://server_iso_docs/...` paths.
