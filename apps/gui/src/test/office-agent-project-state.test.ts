...
describe("OfficeAgent project-scoped tool state", () => {
  it("shares package/cache env paths across sessions in the same project", async () => {
    ...
    try {
      ...
      expect(sessionA.OFFICE_AGENT_TOOL_FILES).toBe(path.join(projectDir, "officeagent-tool-files"));
      expect(sessionA.OFFICE_AGENT_TOOL_FILES).toBe(sessionB.OFFICE_AGENT_TOOL_FILES);
      ...
      expect(sessionA.OFFICE_AGENT_TOOL_FILES).not.toBe(sessionC.OFFICE_AGENT_TOOL_FILES);
