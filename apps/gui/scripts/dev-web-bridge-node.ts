...
const handlers: DesktopRequestHandlerMap = {
  ...
  getWindowsSandboxSetupStatus: () => ({
    available: false,
    ready: false,
    status: "unavailable-in-dev-bridge",
    issues: ["Windows sandbox setup is only available in Electron."],
  }),
  prepareWindowsSandboxSetup: ({ action }) => ({
    ok: false,
    action: action ?? "setup",
    error: "Windows sandbox setup is only available in Electron.",
  }),
  runWindowsSandboxSetup: ({ action }) => ({
    ok: false,
    action: action ?? "setup",
    error: "Windows sandbox setup is only available in Electron.",
  }),
